...
\]
\subparagraph{
Para o espa\c{c}o vamos usar o 99. Avisamos que esta \'e uma tabela apenas com fim did\'atico, e, por isso h\'a v\'arios caracteres desconsiderados. Como exemplo vamos pr\'e-encriptar o poema Amor, de Oswald de Andrade. O texto do poema a ser encriptado é o  pr\'e-seguinte:
}
\[
\begin{array}{ccccccccccccc}
Amor  \\ 
...
\[
\begin{array}{c}
\textit{$102$ | $224$ | $279$ | $91$ | $7$ | $30$ | $222$ | $42$ | $7$}
\end{array}
\]
\subparagraph{
...
\subparagraph{
Encerrada a fase de pr\'e-codificando vamos agora codificar nossas mensagens. Manteremos os valores e exemplos da seção anterior a fim de facilitar a compreens\~ao.
}
...
\[
\begin{array}{c}
\textit{$C(b) \equiv b^3 (mod n)$}
\end{array}
\]
\subparagraph{
Podemos para facilitar dizer que $C(b)$ é o resíduo de $b^3$ pelo m\'odulo $n$. Vamo a uma demonstra\c{c}\~ao pr\'atica com o primiro bloco de nossa mensagem, com o n\'umero $102$. Para simplificar o nosso trabalho vamos utilizar as opera\c{c}\~oes modulares.
}
\[
\begin{array}{c}
\textit{$102^3 \equiv 24276 \equiv 34 (mod 391)$}
\end{array}
\]
\subparagraph{
...
\[
\begin{array}{c}
\textit{$224^3 \equiv 11239424 \equiv 129 (mod 391) $}\\
\textit{$279^3 \equiv 21717639 \equiv 326 (mod 391) $}\\
\textit{$91^3  \equiv 753571   \equiv 114 (mod 391) $}\\
\textit{$7^3   \equiv 343      \equiv 343 (mod 391) $}\\
\textit{$30^3  \equiv 27000    \equiv 21  (mod 391) $}\\
\textit{$222^3 \equiv 10941048 \equiv 86  (mod 391) $}\\
\textit{$42^3  \equiv 74088    \equiv 189 (mod 391) $}\\
\textit{$7^3   \equiv 343      \equiv 343 (mod 391) $}\\
\end{array}
\]
\subparagraph{
...
\[
\begin{array}{c}
 \textit{ $34$ | $129$ | $326$ | $114$ | $343$ | $21$ | $86$ | $189$ | $343$}
\end{array}
\]

\subsection{Decodificando uma mensagem}
\subparagraph{
Para podermos desencriptar uma mensagem n\'os precisamos de dois números. O primeiro \'e a nossa chave p\'ublica $n$. O segundo n\'umero \'e $d$, o $d$ é o inverso de $3$ para o m\'odulo $(p-1)(q-1)$. Mais a frente explicaremos o motivo de se adicionar este n\'umero. Pela defini\c{c}\~ao do inverso temos:
}
\[
\begin{array}{c}
...
\[
\begin{array}{c}
\textit{$(p-1)(q-1) \equiv 4 \cdot 4 \equiv 16 \equiv 4 \equiv -2(mod 6)$}
\end{array}
\]
\paragraph{
ou seja:
...
\[
\begin{array}{c}
\textit{$(p-1)(q-1) = 16 \cdot 22 = 352 = 6 \cdot 58 + 4 = 6\cdot 59 -2$}
\end{array}
\]
\paragraph{
...
\[
\begin{array}{c}
\textit{$34 \equiv 0 (mod 17)$}\\
\textit{$34 \equiv 11(mod 23)$}\\
\end{array}
\]
\subparagraph{
...
\[
\begin{array}{c}
\textit{$11^{235} \equiv (11^{22})^{10} \cdot 11^{15} \equiv 11^{15}(mod 23)$}
\end{array}
\]
\subparagraph{
...
\[
\begin{array}{c}
\textit{$11^{235} \equiv 11^{15} \equiv -4^{15} \cdot 3^{15}(mod 23)$}
\end{array}
\]
\paragraph{
Com isso, teremos:
...
\[
\begin{array}{c}
\textit{$415 \equiv 230 \equiv (2^{11})^2 \cdot 2^8 \equiv 2^8 \equiv 3 (mod 23)$}\\
\textit{$315 \equiv 3^{11} \cdot 3^4 \equiv 3^4 \equiv 12 (mod 23)$}
\end{array}
\]
\paragraph{
Concluindo assim:
...
\[
\begin{array}{c}
\textit{$11235 \equiv -415 \cdot 315 \equiv -3 \cdot 12 \equiv 10 (mod 23)$}
\end{array}
\]
\subparagraph{
...
\[
\begin{array}{c}
\textit{$x \equiv 0 (mod 17)$}\\
\textit{$x \equiv 10 (mod 23)$}\\
\end{array}
\]
\subparagraph{
...
\[
\begin{array}{c}
\textit{$y \equiv 3 \cdot 7 \equiv 4 (mod 17)$}\\
\end{array}
\]
\subparagraph{
...
\[
\begin{array}{c}
\textit{$b \equiv DC(b) (mod n)$}
\end{array}
\]
\subparagraph{
N\'os j\'a vimos que $C(b) \equiv b^3 (mod n)$ e $D(a) \equiv a^d(mod n)$. Se combinarmos ambos as congru\^encia iremos obter
}
\[
\begin{array}{c}
\textit{$D(C(b)) \equiv D(b^3) \equiv b^{3d}(mod n)$}
\end{array}
\]
\subparagraph{
...
\[
\begin{array}{c}
\textit{$b^{3d} \equiv b(b^{p-1})^{k(q-1)}$}
\end{array}
\]
\subparagraph{
...
\[
\begin{array}{c}
\textit{$b^{3d} \equiv b \cdot (1)^{k(q-1)}\equiv b (mod p)$}
\end{array}
\]
\subparagraph{
Mesma que $b$ seja m\'ultiplo de $p$, teremos que $b$ e $b^{3d}$ s\~ao congruentes a $0$, logo nessa caso tamb\'em \'e v\'alida a congru\^encia, tendo assim: 
}
\[
\begin{array}{c}
\textit{$b^{3d} \equiv b (mod p)$}
\end{array}
\]
\subparagraph{
...
\[
\begin{array}{c}
\textit{$x \equiv b (mod p)$}\\
\textit{$x \equiv b (mod q)$}\\
\end{array}
\]
\paragraph{
...
\[
\begin{array}{c}
\textit{$b^{3d} \equiv b + p \cdot q \cdot k$}\\
\end{array}
\]
\paragraph{
para algum inteiro $k$. Isso equivale a $b^{3d} \equiv b (mod (pq))$. Isso comprova a congru\^encia que quer\'iamos a efici\^encia do RSA.
}
\section{Discutindo a seguran\c{c}a do RSA}
\subparagraph{
Antes de mudarmos completamente o foco deste trabalho, vamos nos focar no que tange a seguran\c{c}a do RSA. Vamos supor que algu\'em, que vamos denominar Irineu, esteja com uma escuta em nossas mensagens, tendo assim acesso tanto a mensagem codificada quanto a chave p\'ublica $n$. Vamos lembrar que $n$ \'e a multiplica\c{c}\~ ao dos primos $p$ e $q$. Sabendo disso bastaria para Irineu fatorar $n$ para obter $p$ e $q$ e depois descobrir $d$ para poder decodificar a mensagem, como j\'a foi explicado neste cap\'itulo.
}
\subparagraph{
